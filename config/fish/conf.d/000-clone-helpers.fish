...
function clone-work --description "clone-work <repo>"
    set repo $argv[1]; and set --erase argv[1];
    git clone git@github.com:Khan/$repo $HOME/Code/github.com/Khan/$repo $argv
    git config --file $HOME/Code/github.com/Khan/$repo/.git/config --add user.email 'alexvidal@khanacademy.org'
end
